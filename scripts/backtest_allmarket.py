...
class StrategyBacktester:
    ...
    def load_and_aggregate_trend(self, code):
        ...
        try:
            ...
            daily = df.groupby('day').agg({'趋势代码': 'last'}).reset_index()
    ...
    def generate_report(self, all_results, holding_periods):
        ...
        report_file = os.path.join(self.results_dir, f"全市场策略回测_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
        
        with open(report_file, 'w', newline='', encoding='utf-8-sig') as f:
            writer = csv.writer(f)
            writer.writerow(['策略回测结果 - 全市场'])
            writer.writerow([f'回测时间: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'])
            writer.writerow([f'股票数量: 3667只'])
            writer.writerow([])
