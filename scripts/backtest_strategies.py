...
class StrategyBacktester:
    ...
    def __init__(self):
        self.trend_dir = os.path.join(BASE_DIR, "output_allmarket", "趋势判断")
    ...
    def load_and_aggregate_trend(self, code):
        """加载趋势数据并聚合到日级别"""
        trend_file = os.path.join(self.trend_dir, f"{code}_趋势判断.csv")
    ...
    def load_indicator(self, code):
        ...
        indicator_file = os.path.join(self.indicator_dir, f"{code_normalized}_indicators.csv")
    ...
    def load_price(self, code):
        """加载价格数据"""
        code_normalized = self.normalize_code(code)
        price_file = os.path.join(self.price_dir, f"{code_normalized}_day.csv")
    ...
    def run_backtest(self):
        ...
        trend_files = [f for f in os.listdir(self.trend_dir) if f.endswith('_趋势判断.csv')]
        total = len(trend_files)
        
        print(f"开始回测，共 {total} 只股票...")
        print(f"持有期限: {holding_periods}")
        print()
        
        for idx, trend_file in enumerate(trend_files):
            code = trend_file.replace('_趋势判断.csv', '')
            
            trend_df = self.load_and_aggregate_trend(code)
    ...
    def generate_report(self, all_results, holding_periods):
        ...
        report_file = os.path.join(self.results_dir, f"策略回测报告_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
        
        with open(report_file, 'w', newline='', encoding='utf-8-sig') as f:
            writer = csv.writer(f)
            
            writer.writerow(['策略回测结果', '', '', '', '', ''])
