...
class RecordsGenerator:
    ...
    def generate_stock(self, symbol):
        ...
        prev_trend = None
        count = 0
        max_records = 30  # 最多记录30个趋势变化点

        for idx, row in df.iterrows():
            current_trend = row["趋势代码"]
            current_price = row["当前价格"]

            # 检测趋势变化
            if prev_trend is not None and current_trend != prev_trend and count < max_records:
                key_price = self._get_key_price(current_trend, row)
                ratio = self._calculate_ratio(current_price, key_price)

                # 创建记录
                record = {
                    "时间": row["时间"],
                    "当前价格": current_price,
                    "趋势": TREND_COLUMNS.get(current_trend, current_trend),
                    "趋势代码": current_trend,
                    "关键价格": key_price,
                    "比值": ratio,
                }
                ...
                # 添加所有可能的趋势关键价格
                for code, col_name in TREND_COLUMNS.items():
                    val = row.get(f"{code.replace('_', '_')}".replace("up_", "rally_")
                                 .replace("down_", "rally_").replace("_", "_"))
                    # 直接从row获取
                    pass
